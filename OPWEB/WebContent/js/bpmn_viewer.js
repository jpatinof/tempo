/*!
 * bpmn-js - bpmn-navigated-viewer v6.3.1
 *
...
(function (global, factory) {
	...
	/**
	 * Iterate over collection; returning something (non-undefined) will stop
	 * iteration.
	 * 
	 * @param {Array|Object}
	 *            collection
	...
	/**
	 * Return true if every element in the collection matches the criteria.
	 * 
	 * @param {Object|Array}
	 *            collection
	 * @param {Function}
	...
	/**
	 * Return true if some elements in the collection match the criteria.
	 * 
	 * @param {Object|Array}
	 *            collection
	 * @param {Function}
	...
	/**
	 * Transform a collection into another collection by piping each member
	 * through the given fn.
	 * 
	 * @param {Object|Array}
	 *            collection
	 * @param {Function}
	...
	/**
	 * Debounce fn, calling it only once if the given time elapsed between
	 * calls.
	 * 
	 * @param {Function}
	 *            fn
	...
	/**
	 * Should check whether *this* renderer can render the element/connection.
	 * 
	 * @param {element}
	 *            element
	 * 
	...
	/**
	 * Remove class `name` when present, or pass a regular expression to remove
	 * any which match.
	 * 
	 * @param {String|RegExp}
	 *            name
	 ...
	 * 
	 * @param {String}
	 *            name the name or markup of the element
	 * @param {Object}
	 ...
	 * @example
	 * 
	 * createMatrix({ a: 1, b: 1 }); createMatrix(); createMatrix(1, 2, 0, 0,
	 * 30, 20);
	 * 
	 * @return {SVGMatrix}
	 */
	...
	/**
	 * innerHTML like functionality for SVG elements. based on innerSVG
	 ...
	 * @param {SVGElement}
	 *            node
	 * @param {SVGTransform|SVGMatrix|Array
	 *            <SVGTransform|SVGMatrix>} [transforms]
	 * 
	...
	/**
	 * Shortens a line based on spacing and hyphens. Returns the shortened
	 * result on success.
	 * 
	 * @param {String}
	 *            line
	 ...
	 * @param {boolean}
	 *            options.fitBox indicates if box will be recalculated to fit
	 *            text
	 * 
	...
	Text.prototype.layoutText = function (text, options) {
		...
		// ensure correct rendering by attaching helper text node to invisible
		// SVG
	...
	function PathMap() {
		...
		/**
		   ...
		   * 
		   * <p>
		   * It's important to specify a correct <b>height and width</b> for the
		   * path as the scaling is based on the ratio between the specified
		   * height and width in this object and the height and width that is set
		   * as scale target (Note x,y coordinates will be scaled with individual
		   * ratios).
		   * </p>
		   * <p>
		   ...
		   * <ul>
		   * <li>The values for the y axis can be accessed in the path string
		   * using {e.y0}, {e.y1}, ....</li>
		   * <li>The values for the x axis can be accessed in the path string
		   * using {e.x0}, {e.x1}, ....</li>
		   * </ul>
		...
		/**
		   ...
		   * <p>
		   * Scaling a path with transform() will also scale the stroke and IE
		   * does not support the option 'non-scaling-stroke' to prevent this.
		   ...
		   * <li>Position is used to set the starting coordinate of the path. M
		   * is computed:
		   * <ul>
	...
	/**
	 * A simple translation stub to be used for multi-language support in
	 * diagrams. Can be easily replaced with a more sophisticated solution.
	 * 
	 * @example // use it inside any diagram component by injecting `translate`.
	 * 
	 * function MyService(translate) { alert(translate('HELLO {you}', { you:
	 * 'You!' })); }
	 * 
	 * @param {String}
	 *            template to interpolate
	 * @param {Object}
	 *            [replacements] a map with substitutes
	 * 
	...
	/**
	 * This file contains source code adapted from Snap.svg (licensed
	 * Apache-2.0).
	 * 
	...
	/**
	 * Add bpmn element (semantic) to the canvas onto the specified parent
	 * shape.
	 */
	BpmnImporter.prototype.add = function (semantic, parentElement) {
		...
		// SHAPE
			...
			if (is(semantic, 'bpmn:DataStoreReference')) {
				...
				// check whether data store is inside our outside of its semantic
				// parent
	 ...
	 * 
	 * It emits the following events: * element.click * element.contextmenu *
	 * element.dblclick * element.hover * element.mousedown * element.mousemove *
	 * element.mouseup * element.out
	 * 
	...
	function InteractionEvents(eventBus, elementRegistry, styles) {
		...
		/**
		   ...
		   * 
		   * @param {String}
		   *            type local event name, e.g. element.click.
		   * @param {DOMEvent}
		   *            event native event
		   * @param {djs.model.Base}
		   *            [element] the diagram element to emit the event on;
		...
		/**
		   * Trigger an interaction event (based on a native dom event) on the
		   * target shape or connection.
		   * 
		   * @param {String}
		   *            eventName the name of the triggered DOM event
		   * @param {MouseEvent}
		   *            event
		   * @param {djs.model.Base}
		   *            targetElement
		...
		// A low priortity is necessary, because djs-hit of labels has to be
		// updated
	 ...
	 * 
	 * @param {Array
	 *            <djs.model.Shape>|djs.model.Shape} elements
	 * @param {Boolean}
	 *            stopRecursion
	 */
	 ...
	 * @class
	 * 
	 * A plugin that adds an outline to shapes and connections that may be
	 * activated and styled via CSS classes.
	 * 
	 * @param {EventBus}
	 *            eventBus
	 * @param {Styles}
	...
	function Outline(eventBus, styles, elementRegistry) {
		...
		// A low priortity is necessary, because outlines of labels have to be
		// updated
		// after the label bounds have been updated in the renderer.
	...
	/**
	 * Updates the outline of a shape respecting the dimension of the element
	...
	/**
	 * Updates the outline of a connection respecting the bounding box of the
	 * connection and an outline offset.
	 * 
	 * @param {SVGElement}
	 *            outline
	 * @param {djs.model.Base}
	...
	/**
	 * A service that offers the current selection in a diagram. Offers the api
	 * to control the selection, too.
	 * 
	 * @class
	 * 
	 * @param {EventBus}
	...
	function SelectionVisuals(events, canvas, selection, styles) {
		...
		events.on('element.hover', function (event) {
			accionHoverTooltip(event.element);
			addMarker(event.element, MARKER_HOVER);
		});

		events.on('element.out', function (event) {
			accionOutTooltip(event.element);
	 ...
	 * @memberOf djs.util
	 * 
	 * The ids can be customized via a given prefix and contain a random value
	 * to avoid collisions.
	 * 
	 * @param {String}
	...
	function activateDirectEdit(element, force) {
		...
		if (esAlguno(element, ['bpmn:Task','bpmn:UserTask','bpmn:ReceiveTask','bpmn:ServiceTask', 'bpmn:SequenceFlow', 'label'])) {
			accionInformacion(element);
		}
	}
	/**
	 * Retorna True si el elemento pertenece al array enviado
	 */
	function esAlguno(element, valores) {
		var returnValue;
		returnValue = false;
		if (element != null && isValidCollection(valores)) {
			valores.forEach((item) => {
				if (item != null) {
					if (element.businessObject.$type == item && !returnValue) {
						returnValue = true;
					}
				}
			})
	 ...
	 * 
	 * @example // add a pink badge on the top left of the shape
	 *          overlays.add(someShape, { position: { top: -5, left: -5 }, html: '<div
	 ...
	 * 
	 * 
	 * You may configure overlay defaults during tool by providing a `config`
	 * module with `overlays.defaults` as an entry: { overlays: { defaults: {
	 * show: { minZoom: 0.7, maxZoom: 5.0 }, scale: { min: 1 } } }
	 * 
	 * @param {Object}
	 *            config
	 * @param {EventBus}
	 ...
	 * 
	 * @example // return the single overlay with the given id
	 *          overlays.get('some-id'); // return all overlays for the shape
	 *          overlays.get({ element: someShape }); // return all overlays on
	 *          shape with type 'badge' overlays.get({ element: someShape, type:
	 *          'badge' }); // shape can also be specified as id overlays.get({
	 *          element: 'element-id', type: 'badge' });
	 * 
	 * 
	 * @param {Object}
	 *            search
	 ...
	 * @param {Number}
	 *            [overlay.position.left] relative to element bbox left
	 *            attachment
	 * @param {Number}
	 ...
	 *            attachment
	 * @param {Number}
	 *            [overlay.position.right] relative to element bbox right
	 *            attachment
	 * @param {Boolean|Object}
	 *            [overlay.scale=true] false to preserve the same size
	 *            regardless of diagram zoom
	 * @param {Number}
	 *            [overlay.scale.min]
	 * @param {Number}
	 *            [overlay.scale.max]
	 * 
	 * @return {String} id that may be used to reference the overlay for update
	 *         or removal
	 */
	...
	function Injector(modules, parent) {
		...
		/**
		   ...
		   *            name
		   * @param {Boolean}
		   *            [strict=true] if false, resolve missing services to null
		   * 
		   * @return {Object}
		   */
	...
	/**
	 * Fail save add an element to the given connection, ensuring it does not
	 * yet exist.
	 * 
	 * @param {Array
	 *            <Object>} collection
	...
	function createInfoBox(options) {
		...
		// (this is impossible for <svg> elements at the moment)
		var parent = document.createElement('div');
		parent.setAttribute('id', 'infoProcesos');
		parent.setAttribute('class', 'notice');
	...
	function createInfoColorsBox(options) {
		...
		// (this is impossible for <svg> elements at the moment)
		var parent = document.createElement('div');
		parent.setAttribute('id', 'infoColors');
		parent.setAttribute('class', 'notice');
	...
	function createRefreshButton(options) {
		...
		// (this is impossible for <svg> elements at the moment)
		var parent = document.createElement('div');
		parent.setAttribute('id', 'refreshButton');
		parent.setAttribute('class', 'notice');
	...
	Canvas.prototype._init = function (config) {
		...
		// This way we are always able to correctly figure out the size of the
		// svg element
		...
		eventBus.on('diagram.init', function () {
			 ...
			 * @property {SVGElement} viewport the direct parent of diagram elements
			 *           and shapes
			 */
	 ...
	 * 
	 * Non-existing layers retrieved through this method will be created. During
	 * creation, the optional index may be used to create layers below or above
	 * existing layers. A layer with a certain index is always created above all
	...
	/**
	 * Sets a given element as the new root element for the canvas and returns
	 * the new root element.
	 * 
	 * @param {Object|djs.model.Root}
	 *            element
	 * @param {Boolean}
	 *            [override] whether to override the current root element, if
	 *            any
	 * 
	 ...
	 * 
	 * During add it emits the events * <{type}.add> (element, parent) *
	...
	Canvas.prototype.removeShape = function (shape) {
		...
		/**
		   * An event indicating that a shape is about to be removed from the
		   * canvas.
		   * 
		   * @memberOf Canvas
		   * 
	...
	Canvas.prototype.removeConnection = function (connection) {
		...
		/**
		   * An event indicating that a connection has been removed from the
		   * canvas.
		   * 
		   * @memberOf Canvas
		   * 
	 ...
	 * displayed.
	 * 
	 * The getter may return a cached viewbox (if it is currently changing). To
	 * force a recomputation, pass `false` as the first argument.
	 * 
	 * @example
	 * 
	 * canvas.viewbox({ x: 100, y: 100, width: 500, height: 500 }) // sets the
	 * visible area of the diagram to (100|100) -> (600|100) // and and scales
	 * it according to the diagram width
	 * 
	 * var viewbox = canvas.viewbox(); // pass `false` to force recomputing the
	 * box.
	 * 
	 * console.log(viewbox); // { // inner: Dimensions, // outer: Dimensions, //
	 * scale, // x, y, // width, height // } // if the current diagram is zoomed
	 * and scrolled, you may reset it to the // default zoom via this method,
	 * too:
	 * 
	 * var zoomedAndScrolledViewbox = canvas.viewbox();
	 * 
	 * canvas.viewbox({ x: 0, y: 0, width: zoomedAndScrolledViewbox.outer.width,
	 * height: zoomedAndScrolledViewbox.outer.height });
	 * 
	 * @param {Object}
	 *            [box] the new view box to set
	 * @param {Number}
	 *            box.x the top left X coordinate of the canvas visible in view
	 *            box
	 * @param {Number}
	 *            box.y the top left Y coordinate of the canvas visible in view
	 *            box
	 * @param {Number}
	 ...
	 * specified position.
	 * 
	 * The getter may return a cached zoom level. Call it with `false` as the
	 * first argument to force recomputation of the current level.
	 * 
	 * @param {String|Number}
	 *            [newScale] the new zoom level, either a number, i.e. 0.9, or
	 *            `fit-viewport` to adjust the size to fit the current viewport
	 * @param {String|Point}
	 *            [center] the reference point { x: .., y: ..} to zoom to,
	 *            'auto' to zoom into mid or null
	 * 
	 ...
	 * (browser) coordinate system rather than the zoomed in/out canvas
	 * coordinates.
	 * 
	...
	/**
	 * Fires an event in order other modules can react to the canvas resizing
	 */
	 ...
	 *            gfx
	 * @param {SVGElement}
	 *            [secondaryGfx] optional other element to register, too
	 */
	...
	function extend$1(collection, refs, property, target) {
		...
		/**
		   * Adds an element to the array, unless it exists already (set
		   * semantics).
		   * 
		   ...
		   * @param {Number}
		   *            optional index to add element to (possibly moving other
		   *            elements around)
		   */
	 ...
	 * 
	 * <p>
	 * When bound to an object using {@link Refs#bind} the references get
	 * activated and ensure that add and remove operations are applied
	 * reversely, too.
	 * </p>
	 * 
	 * <p>
	 ...
	 * {@link RefsCollection#contains} extensions that must be used to properly
	 * hook into the inverse change mechanism.
	 * </p>
	 * 
	 * @class Refs
	 * 
	 ...
	 * @example
	 * 
	 * var shape1 = Model.create('shape', { x: 10, y: 10, width: 100, height:
	 * 100 }); var shape2 = Model.create('shape', { x: 210, y: 210, width: 100,
	 * height: 100 });
	 * 
	 * var connection = Model.create('connection', { waypoints: [ { x: 110, y:
	 * 55 }, {x: 210, y: 55 } ] });
	 * 
	 * @param {String}
	 ...
	 * 
	 * This component is used to communicate across a diagram instance. Other
	 * parts of a diagram can use it to listen to and broadcast events. ##
	 * Registering for Events
	 * 
	 * The event bus provides the {@link EventBus#on} and {@link EventBus#once}
	 * methods to register for events. {@link EventBus#off} can be used to
	 * remove event registrations. Listeners receive an instance of
	 * {@link Event} as the first argument. It allows them to hook into the
	 * event execution.
	 * 
	 * ```javascript // listen for event eventBus.on('foo', function(event) { //
	 * access event type event.type; // 'foo' // stop propagation to other
	 * listeners event.stopPropagation(); // prevent event default
	 * event.preventDefault(); }); // listen for event with custom payload
	 * eventBus.on('bar', function(event, payload) { console.log(payload); }); //
	 * listen for event returning value eventBus.on('foobar', function(event) { //
	 * stop event propagation + prevent default return false; // stop event
	 * propagation + return custom result return { complex: 'listening result' };
	 * }); // listen with custom priority (default=1000, higher is better)
	 * eventBus.on('priorityfoo', 1500, function(event) { console.log('invoked
	 * first!'); }); // listen for event and pass the context (`this`)
	 * eventBus.on('foobar', function(event) { this.foo(); }, this); ``` ##
	 * Emitting Events
	 * 
	 * Events can be emitted via the event bus using {@link EventBus#fire}.
	 * 
	 * ```javascript // false indicates that the default action // was prevented
	 * by listeners if (eventBus.fire('foo') === false) { console.log('default
	 * has been prevented!'); }; // custom args + return value listener
	 * eventBus.on('sum', function(event, a, b) { return a + b; }); // you can
	 * pass custom arguments + retrieve result values. var sum =
	 ...
	 * 
	 * The callback will be invoked with `event, ...additionalArguments` that
	 * have been passed to {@link EventBus#fire}.
	 * 
	 ...
	 *            callback
	 * @param {Object}
	 *            [that] Pass context (`this`) to the callback
	 */
	 ...
	 * Fires a named event.
	 * 
	 * @example // fire event by name events.fire('foo'); // fire event object
	 *          with nested type var event = { type: 'foo' };
	 *          events.fire(event); // fire event with explicit type var event = {
	 *          x: 10, y: 20 }; events.fire('element.moved', event); // pass
	 *          additional arguments to the event events.on('foo',
	 *          function(event, bar) { alert(bar); });
	 * 
	...
	/*
	 * Add new listener with a certain priority to the list of listeners (for
	 * the given event).
	 * 
	 * The semantics of listener registration / listener execution are first
	 * register, first serve: New listeners will always be inserted after
	 * existing listeners with the same priority.
	 * 
	 * Example: Inserting two listeners with priority 1000 and 1300 * before: [
	 * 1500, 1500, 1000, 1000 ] * after: [ 1500, 1500, (new=1300), 1000, 1000,
	 * (new=1000) ]
	 * 
	 ...
	 * <g class="djs-group">
	 * 
	 * <!-- the gfx --> <g class="djs-element djs-(shape|connection|frame)"> <g
	 ...
	 *            [childrenGfx]
	 * @param {Number}
	 *            [parentIndex] position to create container in parent
	 * @param {Boolean}
	 *            [isFrame] is frame element
	 * 
	 * @return {SVGElement}
	 */
	...
	/**
	 * The main diagram-js entry point that bootstraps the diagram with the
	 * given configuration.
	 * 
	 * To register extensions with the diagram, pass them as Array<didi.Module>
	 * to the constructor.
	 * 
	 * @class djs.Diagram
	 * @memberOf djs
	 ...
	 * canvas.</caption> // plug-in implemenentation function
	 * MyLoggingPlugin(eventBus) { eventBus.on('shape.added', function(event) {
	 * console.log('shape ', event.shape, ' was added to the diagram'); }); } //
	 * export as module export default { __init__: [ 'myLoggingPlugin' ],
	 * myLoggingPlugin: [ 'type', MyLoggingPlugin ] }; // instantiate the
	 * diagram with the new plug-in
	 * 
	 * import MyLoggingModule from 'path-to-my-logging-plugin';
	 * 
	 * var diagram = new Diagram({ modules: [ MyLoggingModule ] });
	 * 
	 * diagram.invoke([ 'canvas', function(canvas) { // add shape to drawing
	 * canvas canvas.addShape({ x: 10, y: 10 }); }); // 'shape ... was added to
	 * the diagram' logged to console
	 * 
	 * @param {Object}
	 *            options
	 * @param {Array
	 *            <didi.Module>} [options.modules] external modules to
	 *            instantiate with the diagram
	 * @param {didi.Injector}
	 *            [injector] an (optional) injector to bootstrap the diagram
	 *            with
	 */
	...
	/**
	 * Parses a namespaced attribute name of the form (ns:)localName to an
	 * object, given a default prefix to assume in case no explicit namespace is
	 * given.
	 * 
	 * @param {String}
	 *            name
	 * @param {String}
	 *            [defaultPrefix] the default prefix to take, if none is
	 *            present.
	 * 
	...
	/**
	 * Traverse the type hierarchy from bottom to top, calling iterator with
	 * (type, inherited) for all elements in the inheritance chain.
	 * 
	 * @param {Object}
	 *            nsName
	 * @param {Function}
	...
	/**
	 * Sets a named property on the target element. If the value is undefined,
	 * the property gets deleted.
	 * 
	 * @param {Object}
	 *            target
	 * @param {String}
	 ...
	 * @example
	 * 
	 * var foo = moddle.create('my:Foo'); var bar = moddle.create('my:Bar', {
	 * id: 'BAR_1' });
	 * 
	 * @param {String|Object}
	 *            descriptor the type descriptor or name know to the model
	 * @param {Object}
	 *            attrs a number of attributes to initialize the model instance
	 *            with
	 ...
	 * 
	 * This can be used to create custom elements that lie outside the
	 * meta-model. The created element contains all the meta-data required to
	 * serialize it as part of meta-model elements.
	 * 
	 ...
	 * @param {Object}
	 *            [properties] a map of properties to initialize the instance
	 *            with
	...
	/**
	 * Returns true if the given descriptor or instance represents the given
	 * type.
	 * 
	...
	/**
	 * A basic entity decoder that can decode a minimal sub-set of reserved
	 * names (&amp;) as well as hex (&#xaaf;) and decimal (&#1231;) encoded
	 * characters.
	 * 
	 * @param {string}
	 *            str
	...
	function Parser(options) {
		...
		/**
		   ...
		   * 
		   * This map will ensure we can normalize prefixes during processing; for
		   * each uri, only one prefix will be exposed to the handlers.
		   * 
		...
		function parse(xml) {
			 ...
			 * 
			 * Return semantics: (1) `false` on attribute parse error, (2) object
			 * hash on extracted attrs.
			 * 
			...
			/**
			 * Extract the parse context { line, column, part } from the current
			 * parser position.
			 * 
			...
			while (j !== -1) {
				...
				if (w === 47) { // </...
					...
					if (!(w > 96 && w < 123 || w > 64 && w < 91 || w === 95 || w === 58)) { // char
						// 95"_"
						// 58":"
	 ...
	 *            options.rootHandler the root handler for parsing a document
	 * @param {boolean}
	...
	/**
	 * A simple pass through handler that does nothing except for ignoring all
	 * input it receives.
	 * 
			...
			if (prop && prop.isReference) {
				...
				if (prop) {
					...
					if (name !== 'xmlns') {
						...
						// if that is the case we emit a warning to indicate potential
						// misuse
	...
	/**
	 * An element handler that performs special validation to ensure the node it
	 * gets initialized with matches the handlers type (namespace wise).
	 * 
	 * @param {Moddle}
	 *            model
	 * @param {String}
	 ...
	 * @param {String}
	 *            xml
	 * @param {ElementHandler|Object}
	 *            options or rootHandler
	 * @param {Function}
	 *            done
	 */
	...
	/**
	 * A sub class of {@link Moddle} with support for import and export of BPMN
	 * 2.0 xml files.
	 * 
	 * @class BpmnModdle
	 * @extends Moddle
	 ...
	 * @param {Function}
	 *            done callback invoked with (err, xmlStr) once the import
	 *            completes
	 */
	...
	function BpmnTreeWalker(handler, translate) {
		...
		/**
		   ...
		   * 
		   * @param {ModdleElement}
		   *            definitions to walk and import
		   * @param {ModdleElement}
		   *            [diagram] specific diagram to import and display
		   * 
		   * @throws {Error}
		   *             if no diagram to display could be found
		   */
		function handleDefinitions(definitions, diagram) {
			...
			if (is$1(rootElement, 'bpmn:Process')) {
				...
				// force drawing of everything not yet drawn that is part of the
				// target DI
	 ...
	 * @param {ModdleElement
	 *            <BPMNDiagram>} [bpmnDiagram] the diagram to be rendered (if
	 *            not provided, the first one will be rendered)
	 * @param {Function}
	 *            done the callback, invoked with (err, [ warning ]) once the
	 *            import is done
	 */
	function importBpmnDiagram(diagram, definitions, bpmnDiagram, done) {
		...
		/**
		   * Walk the diagram semantically, importing (=drawing) all elements you
		   * encounter.
		   * 
		   * @param {ModdleElement
		   *            <Definitions>} definitions
	 ...
	 * 
	 * Have a look at {@link Viewer}, {@link NavigatedViewer} or
	 * {@link Modeler} for bundles that include actual features.
	 * 
	 * @param {Object}
	 ...
	 * @param {Array
	 *            <didi.Module>} [options.modules] a list of modules to override
	 *            the default modules
	 * @param {Array
	 *            <didi.Module>} [options.additionalModules] a list of modules
	 *            to use with the default modules
	 */
	 ...
	 * 
	 * Once finished the viewer reports back the result to the provided callback
	 * function with (err, warnings). ## Life-Cycle Events
	 * 
	 * During import the viewer will fire life-cycle events: *
	 * import.parse.start (about to read model from xml) * import.parse.complete
	 * (model read; may have worked or not) * import.render.start (graphical
	 * import start) * import.render.complete (graphical import finished) *
	 * import.done (everything done)
	 * 
	 * You can use these events to hook into the life-cycle.
	 * 
	 * @param {String}
	 *            xml the BPMN 2.0 xml
	 * @param {ModdleElement
	 *            <BPMNDiagram>|String} [bpmnDiagram] BPMN diagram or id of
	 *            diagram to render (if not provided, the first one will be
	 *            rendered)
	 * @param {Function}
	 *            [done] invoked with (err, warnings=[])
	 */
	 ...
	 * 
	 * During import the viewer will fire life-cycle events: *
	 * import.render.start (graphical import start) * import.render.complete
	 * (graphical import finished)
	 * 
	 * You can use these events to hook into the life-cycle.
	 * 
	 * @param {ModdleElement
	 *            <Definitions>} definitions parsed BPMN 2.0 definitions
	 * @param {ModdleElement
	 *            <BPMNDiagram>|String} [bpmnDiagram] BPMN diagram or id of
	 *            diagram to render (if not provided, the first one will be
	 *            rendered)
	 ...
	 * You can use these events to hook into the life-cycle.
	 * 
	 * @param {String|ModdleElement
	 *            <BPMNDiagram>} [bpmnDiagramOrId] id or the diagram to open
	 * @param {Function}
	 *            [done] invoked with (err, warnings=[])
	 */
	...
	/**
	 * Export the currently displayed BPMN 2.0 diagram as a BPMN 2.0 XML
	 * document. ## Life-Cycle Events
	 * 
	 * During XML saving the viewer will fire life-cycle events: * saveXML.start
	 * (before serialization) * saveXML.serialized (after xml generation) *
	 ...
	 * @param {Boolean}
	 *            [options.format=false] output formatted XML
	 * @param {Boolean}
	 *            [options.preamble=true] output preamble
	 * 
	 * @param {Function}
	 *            done invoked with (err, xml)
	 */
	...
	/**
	 * Export the currently displayed BPMN 2.0 diagram as an SVG image. ##
	 * Life-Cycle Events
	 * 
	 * During SVG saving the viewer will fire life-cycle events: * saveSVG.start
	 * (before serialization) * saveSVG.done (everything done)
	 * 
	 * You can use these events to hook into the life-cycle.
	 * 
	 * @param {Object}
	 ...
	 * @example
	 * 
	 * var elementRegistry = viewer.get('elementRegistry'); var startEventShape =
	 ...
	 * @example
	 * 
	 * viewer.invoke(function(elementRegistry) { var startEventShape =
	 * elementRegistry.get('StartEvent_1'); });
	 * 
	 * @param {Function}
	 *            fn to be invoked
	 ...
	 * 
	 * After calling this method the viewer can still be reused for opening
	 * another diagram.
	 * 
	...
	/**
	 * Destroy the viewer instance and remove all its remainders from the
	 * document tree.
	 */
	...
	/**
	 * Adds the project logo to the diagram container as required by the bpmn.io
	 * license.
	 * 
	 ...
	 * 
	 * Have a look at {@link NavigatedViewer} or {@link Modeler} for bundles
	 * that include additional features. ## Extending the Viewer
	 * 
	 * In order to extend the viewer pass extension modules to bootstrap via the
	 * `additionalModules` option. An extension module is an object that exposes
	 * named services.
	 * 
	 * The following example depicts the integration of a simple logging
	 * component that integrates with interaction events:
	 * 
	 * 
	 * ```javascript // logging component function InteractionLogger(eventBus) {
	 ...
	 * interactionLogger: [ 'type', InteractionLogger ] }; // extend the viewer
	...
	/**
	 * A keyboard abstraction that may be activated and deactivated by users at
	 * will, consuming key events and triggering diagram actions.
	 * 
	 * For keys pressed down, keyboard fires `keyboard.keydown` event. The event
	 * context contains one field which is `KeyboardEvent` event.
	 * 
	 * The implementation fires the following key events that allow other
	 * components to hook into key handling: - keyboard.bind - keyboard.unbind -
	 * keyboard.init - keyboard.destroy
	 * 
	 ...
	 * 
	 * This does not pull in any features will bind only actions that have
	 * previously been registered against the editorActions component.
	 * 
	 * @param {EventBus}
	 *            eventBus
	 * @param {Keyboard}
	...
	KeyboardBindings.prototype.registerBindings = function (keyboard, editorActions) {
		...
		/**
		   ...
		   *            action name
		   * @param {Function}
		   *            fn that implements the key binding
		   */
	 ...
	 *            [config.moveSpeed=50]
	 * @param {Number}
	 *            [config.moveSpeedAccelerated=200]
	 * @param {Keyboard}
	 *            keyboard
	 * @param {Canvas}
	 *            canvas
	...
	/**
	 * An implementation of zooming and scrolling within the {@link Canvas} via
	 * the mouse wheel.
	 * 
	 * Mouse wheel zooming / scrolling may be disabled using the
	 * {@link toggle(enabled)} method.
	 * 
	 * @param {Object}
	 *            [config]
	 * @param {Boolean}
	 *            [config.enabled=true] default enabled state
	 * @param {Number}
	 *            [config.scale=.75] scroll sensivity
	 * @param {EventBus}
	 *            eventBus
	 * @param {Canvas}
	 *            canvas
	 ...
	 * 
	 * @param {number}
	 *            delta - Zoom delta (1 for zooming in, -1 for out).
	 * @param {Object}
	 *            position
	 */
	...
	ZoomScroll.prototype._zoom = function (delta, position, stepSize) {
		...
		// calculate the absolute logarithmic zoom level based on the linear
		// zoom level
